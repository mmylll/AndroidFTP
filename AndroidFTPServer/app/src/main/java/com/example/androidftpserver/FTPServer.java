...
public class FTPServer {
    ...
    private Map<Integer, ClientThread> client_threads = null; //�߳�ӳ��
    private Map<String, String> user = null;
    private int thread_id = 1;
    private int client_count = 0;    //��ǰ�߳�����
    ...
    public void cmdPort(FTPServer server, ClientThread thread, String command, PrintWriter pw){
        if(!loginStatus){
            try {
                String str = command.substring(4).trim();
                int port_low = Integer.parseInt(str.substring(str.lastIndexOf(",")+1));
                int port_high = Integer.parseInt(str.substring(0, str.lastIndexOf(",")).substring(str.substring(0, str.lastIndexOf(",")).lastIndexOf(",")+1));
                String str1 = str.substring(0, str.substring(0, str.lastIndexOf(",")).lastIndexOf(","));
                String retr_ip = str1.replace(",", ".");
                try {
//ʵ��������ģʽ�µ�socket
                    Socket dtp_socket = new Socket(retr_ip,port_high * 256 + port_low);
                    pw.write("200 port command successful");
                    pw.flush();
                    thread.setDtp_socket(dtp_socket);
                    thread.setPortMode(true);
                    thread.setPasvmode(false);
                } catch (ConnectException ce) {
                    pw.write("425 Can't open data connection.");
                    pw.flush();
            ...
            pw.println("530 Please log in with USER and PASS first.");
            pw.flush();
        }
